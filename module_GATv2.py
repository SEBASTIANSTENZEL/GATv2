...
class GATv2Conv(MessagePassing):
    ...
    def __init__(
        ...
        fill_value: Union[float, Tensor, str] = 'add',
        bias: bool =True,
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
        ...
        if self.bias is not None:
            out += self.bias
        # print(out.shape) torch.Size([4093, 100])
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_attr: OptTensor,
        ...
        # x * self.att [50795, 1, 100]
        alpha = (x * self.att).sum(dim=-1)
        alpha = softmax(alpha, index, ptr, size_i)
        # print(alpha.shape) torch.Size([50795, 1])
        self._alpha = alpha #获取权重列表
        alpha = F.dropout(alpha, p=self.dropout, training=self.training) #权重列表激活
